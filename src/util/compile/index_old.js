...
fs.writeFileSync(`./src/util/compile/out/${sourceFileName}.java`, JavaProgram);
JavaProgram = null;
JavaMain = null;
...
execSync(`javac ./src/util/compile/out/${sourceFileName}.java`);

// generate .jar manifest
const manifest = `Manifest-Version: 1.0
Main-Class: ${sourceFileName}\n`;
console.log('generatinf manifest file');
fs.writeFileSync(`./src/util/compile/manifests/${sourceFileName}.mf`, manifest);

// List all .class files
var files = fs.readdirSync(path.resolve('./src/util/compile/out'));
var fullPathFiles = [];
for (let i = 0; i < files.length; i++) {
    const fileName = files[i];
    if (fileName.endsWith('.java')) continue;
    fullPathFiles.push(path.resolve(`./src/util/compile/out/${fileName}`));
}
console.log(fullPathFiles);

// generate .jar file
// console.log('generating .jar');
var fullPath = path.resolve(`./src/util/compile/manifests/${sourceFileName}.mf`);
execSync(`jar cvfm ${sourceFileName}.jar ${fullPath} ${fullPathFiles.join(' ')}`);
process.exit();
